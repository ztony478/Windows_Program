...
{
	CDialog::DoDataExchange(pDX);
	DDX_Text(pDX, IDC_PATH, szUrl);
	DDX_Control(pDX, IDC_PLAYER, RealPlayer);
}
...
BEGIN_MESSAGE_MAP(CRealPlayerDlg, CDialog)
	...
	ON_WM_QUERYDRAGICON()
	//}}AFX_MSG_MAP
	ON_BN_CLICKED(IDC_SET, &CRealPlayerDlg::OnBnClickedSet)
END_MESSAGE_MAP()


// CRealPlayerDlg 消息处理程序
...
void CRealPlayerDlg::OnBnClickedSet()
{
	// 更新控件数据到变量，取得路径
	this->UpdateData () ;
	// 设置Real Player控件的播放路径
	this->RealPlayer.put_Source ( this->szUrl ) ;
	// 播放
	this->RealPlayer.DoPlay () ;
}
