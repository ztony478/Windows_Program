...
static void aes_ecb_encrypt(aes *a,BYTE *buff)
{
    int i,j,k;
    WORD p[4],q[4],*x,*y,*t;
...
#ifdef LOTS_OF_MEMORY
        y[0]=a->fkey[k]^ftable[(BYTE)x[0]]^
             ftable1[(BYTE)(x[1]>>8)]^
             ftable2[(BYTE)(x[2]>>16)]^
             ftable3[x[3]>>24];
        y[1]=a->fkey[k+1]^ftable[(BYTE)x[1]]^
             ftable1[(BYTE)(x[2]>>8)]^
             ftable2[(BYTE)(x[3]>>16)]^
             ftable3[x[0]>>24];
        y[2]=a->fkey[k+2]^ftable[(BYTE)x[2]]^
             ftable1[(BYTE)(x[3]>>8)]^
             ftable2[(BYTE)(x[0]>>16)]^
             ftable3[x[1]>>24];
        y[3]=a->fkey[k+3]^ftable[(BYTE)x[3]]^
             ftable1[(BYTE)(x[0]>>8)]^
             ftable2[(BYTE)(x[1]>>16)]^
             ftable3[x[2]>>24];
#else
        y[0]=a->fkey[k]^ftable[(BYTE)x[0]]^
             ROTL8(ftable[(BYTE)(x[1]>>8)])^
             ROTL16(ftable[(BYTE)(x[2]>>16)])^
             ROTL24(ftable[x[3]>>24]);
        y[1]=a->fkey[k+1]^ftable[(BYTE)x[1]]^
             ROTL8(ftable[(BYTE)(x[2]>>8)])^
             ROTL16(ftable[(BYTE)(x[3]>>16)])^
             ROTL24(ftable[x[0]>>24]);
        y[2]=a->fkey[k+2]^ftable[(BYTE)x[2]]^
             ROTL8(ftable[(BYTE)(x[3]>>8)])^
             ROTL16(ftable[(BYTE)(x[0]>>16)])^
             ROTL24(ftable[x[1]>>24]);
        y[3]=a->fkey[k+3]^ftable[(BYTE)x[3]]^
             ROTL8(ftable[(BYTE)(x[0]>>8)])^
             ROTL16(ftable[(BYTE)(x[1]>>16)])^
...
/* Last Round */ 
    ...
    y[0]=a->fkey[k]^(WORD)fbsub[(BYTE)x[0]]^
         ROTL8((WORD)fbsub[(BYTE)(x[1]>>8)])^
         ROTL16((WORD)fbsub[(BYTE)(x[2]>>16)])^
         ROTL24((WORD)fbsub[x[3]>>24]);
    y[1]=a->fkey[k+1]^(WORD)fbsub[(BYTE)x[1]]^
         ROTL8((WORD)fbsub[(BYTE)(x[2]>>8)])^
         ROTL16((WORD)fbsub[(BYTE)(x[3]>>16)])^
         ROTL24((WORD)fbsub[x[0]>>24]);
    y[2]=a->fkey[k+2]^(WORD)fbsub[(BYTE)x[2]]^
         ROTL8((WORD)fbsub[(BYTE)(x[3]>>8)])^
         ROTL16((WORD)fbsub[(BYTE)(x[0]>>16)])^
         ROTL24((WORD)fbsub[x[1]>>24]);
    y[3]=a->fkey[k+3]^(WORD)fbsub[(BYTE)x[3]]^
         ROTL8((WORD)fbsub[(BYTE)(x[0]>>8)])^
         ROTL16((WORD)fbsub[(BYTE)(x[1]>>16)])^
         ROTL24((WORD)fbsub[x[2]>>24]);
