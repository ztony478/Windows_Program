...
int mr_ps_zzn_mul(_MIPD_ int deg,big *x,big *y,big *z)
{
...
#ifdef MR_FP_ROUNDING
            else mr_mip->w[j]=mr_sdiv(_MIPP_ x[j],p,ip,mr_mip->w1);
#else
  ...
  /* multiply FFTs */
        for (j=0;j<newn;j++)
#ifdef MR_FP_ROUNDING
            imuldiv(mr_mip->w[j],mr_mip->t[i][j],(mr_small)0,p,ip,(mr_small *)&mr_mip->t[i][j]);
#else
        ...
        for (j=0;j<deg;j++)
        {
            ...
            else 
            {
                if (size(x[j])>=0)
                {
                    copy(x[j],mr_mip->w1);
#ifdef MR_FP_ROUNDING
                    mr_mip->w[j]=mr_sdiv(_MIPP_ mr_mip->w1,p,ip,mr_mip->w1);
#else
...
#ifdef MR_FP_ROUNDING
                    mr_mip->w[j]=p-mr_sdiv(_MIPP_ mr_mip->w1,p,ip,mr_mip->w1);
#else
...
/* compute coefficients modulo fft primes */    
    ...
    {                                              
        ...
        for (j=0;j<=degx;j++)
        {
            if (x[j]==NULL) mr_mip->w[j]=0;
#ifdef MR_FP_ROUNDING
            else mr_mip->w[j]=mr_sdiv(_MIPP_ x[j],p,ip,mr_mip->w1);      /* np*np*N/2 muldivs */
#else
...
{ /* "fast" O(n.log n) multiplication */
    ...
    { /* multiply mod each prime */
        ...
        if (x!=y)
        {
            for (i=0;i<yl;i++) work[i]=MR_REMAIN(y->w[i],p);
...
#ifdef MR_FP_ROUNDING
            imuldiv(dptr[i],work[i],(mr_small)0,p,ip,(mr_small *)&dptr[i]);
#else
